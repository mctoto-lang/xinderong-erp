...
function getDbPath(): string {
  ...
  return path.join(dataDir, 'erp.db');
}
...
function initDatabase(): Database.Database {
  ...
  try {
    const columns = database.prepare("PRAGMA table_info(production_order_items)").all() as { name: string }[];
    const hasOutputProductName = columns.some(col => col.name === 'outputProductName');
    if (!hasOutputProductName) {
      database.prepare('ALTER TABLE production_order_items ADD COLUMN outputProductName TEXT').run();
    }
  } catch {
